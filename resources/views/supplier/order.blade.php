						...
						<form method="GET">
							<div class="row">
								...
								<div class="col-md-3 col-sm-3 col-xs-3">
									<div class="filter-btn">
										<a class="button" href="{{route('supplier.order.index')}}">Clear</a>
							...
							<div class="row">
								@foreach($data['orders'] as $key => $order)
									...
									<div class="col-md-6 col-sm-6 col-xs-12">
										<a href="{{route('supplier.order.show',$order->id)}}">
											<div class="single-tender">
											<div class="heading">
												<h2>{{$order->vendor->business_name}}</h2>
												<p class="address"><i class="fas fa-map-marker-alt"></i> &nbsp;{{$order->vendor->address}}</p>
												<p>
													<i class="far fa-calendar-alt"></i> <span>Order Date</span>{{date('d-M-Y',strtotime($order->created_at))}}</p>
											</div>
													...
													<table>
														...
														<tbody>
															@foreach($order->items as $item)
																<tr>
																	<td>{{$item->title}}</td>
																	<td>{{$item->supply_qty}} {{$item->unit}}</td>
																</tr>
															@endforeach
														</tbody>
														<tfoot>
